const translations = {
  de: {
    languageName: 'Deutsch',
    shortName: 'DE',
    switchLabel: 'Sprache auswählen',
    artists: {
      sectionLabel: 'Künstler',
      heading: 'Künstler',
      loading: 'Künstler werden geladen …',
      missingConfig: 'Supabase ist noch nicht konfiguriert. Bitte VITE_SUPABASE_URL und VITE_SUPABASE_ANON_KEY in Vercel setzen.',
      empty: 'Aktuell sind keine Künstler vorhanden.',
      error: 'Die Künstler konnten nicht geladen werden.',
      imageAlt: 'Künstlerbild',
    },
  },
  en: {
    languageName: 'English',
    shortName: 'EN',
    switchLabel: 'Select language',
    artists: {
      sectionLabel: 'Artists',
      heading: 'Artists',
      loading: 'Loading artists …',
      missingConfig: 'Supabase is not configured yet. Please set VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY in Vercel.',
      empty: 'There are currently no artists.',
      error: 'The artists could not be loaded.',
      imageAlt: 'Artist image',
    },
  },
}

export default translations
